...
void updatePID( PIDdata_t *pid )
{
  /* proportional term */ 								
	...
	/* integral term */ 										
	pid->ui = (pid->Out == pid->v1) ? (pid->Ki*pid->up + pid->i1) : pid->i1;  //pid->Ki*(pid->i1 + pid->up)
	pid->ui = sat(pid->ui, pid->Umax, pid->Umin);	
	pid->i1 = pid->ui;
																                    
	/* control output */ 										          
	pid->v1 = pid->Kp*(pid->up + pid->ui);				//pid->Kp*pid->up + pid->ui
	pid->Out= sat(pid->v1, pid->Umax, pid->Umin);						      
	// pid->w1 = (pid->Out == pid->v1) ? 1.0 : 0.0;		
	// pid->i1= sat(pid->i1, pid->Umax/2,pid->Umin/2 );
}
