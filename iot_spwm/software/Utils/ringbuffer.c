...
int32_t ICACHE_FLASH_ATTR rbRead(rb_t *rb, void *data, size_t count)
{
    ...
    if(NULL == data)
    {
		...
		buffer[bufferpointer][0] = count;
		buffer[bufferpointer][1] = (uint32_t)rb->rbHead;
		buffer[bufferpointer][2] = (uint32_t)rb->rbTail;
		buffer[bufferpointer][3] =  rbCanRead(rb);
		buffer[bufferpointer][4] =  rbCapacity(rb);
		buffer[bufferpointer][5] =   (uint32_t)rb->rbBuff;
		bufferpointer++;
...
uint32_t wbuffer[256][6];
uint8_t wbufferpointer;
int32_t ICACHE_FLASH_ATTR rbWrite(rb_t *rb, const void *data, size_t count)
{
    int32_t tailAvailSz = 0;
		wbuffer[wbufferpointer][0] = count;
		wbuffer[wbufferpointer][1] = (uint32_t)rb->rbHead;
		wbuffer[wbufferpointer][2] = (uint32_t)rb->rbTail;
		wbuffer[wbufferpointer][3] =  rbCanWrite(rb);
		wbuffer[wbufferpointer][4] =  rbCapacity(rb);
		wbuffer[wbufferpointer][5] =   (uint32_t)rb->rbBuff;
		wbufferpointer++;
